...
const guest_names = require("./guest_names");
...
function fetchUserInformation(req, res, next) {
    if (req.session.userID) {
        db_1.UserModel.findById(req.session.userID, function (error, user) {
            if (error) {
                console.log(error);
...
router.post("/login", function (req, res, next) {
    db_1.UserModel.getOrCreateWithUsername(req.body.username, req.body.password, function (error, user) {
        if (error) {
...
router.post("/api/bots/create_bot", function (req, res) {
    if (!req.session.user) {
        ...
        db_1.BotModel.createBotForUser(req.session.user, function (error, bot) {
            if (error) {
                res.status(500).send(error.message);
            }
            else {
...
router.get("/api/bots", function (req, res) {
    if (!req.session.user) {
        ...
        db_1.BotModel.find({ owner: req.session.user }, function (error, bots) {
            if (error) {
                res.status(500).send(error.message);
            }
            else {
...
router.post("/api/bots/:botId/change_username", function (req, res) {
    db_1.BotModel.findById(req.params.botId)
        .populate("owner")
...
router.get("/api/users/:userId", function (req, res) {
    ...
    db_1.UserModel.findById(req.params.userId).exec(onUserFind);
});
